...
export class NdeContentIndicatorsCulturalAdviceCustomComponent {
    ...
    // private allProcessStarted = false
    ...
    private async getTalisDataFromAnyApiCalls(listTalisUrls: string[]): Promise<boolean> {
        ...
        if (courseFound === COURSE_READING_FOUND) {
            ...
            // if (isFullDisplayPage() && !this.allProcessStarted) {
            if (isFullDisplayPage()) {
                // console.log('### getTalisDataFromAllApiCalls call B', this.allProcessStarted, this.uuid);
                // this.allProcessStarted = true;
                const pnx = this.hostComponent;
                console.log('### displayCourseReadingListIndicator pnx=', pnx);

                // const libraryId = document.querySelector('[data-qa="detail_mms"] .mat-body-medium');
                // const libraryIdContent = libraryId?.innerHtml
                // console.log('### libraryIdContent=', libraryIdContent);
                // const recordIdElement = item?.querySelector( '[data-recordid]') || item?.querySelector( 'a[ng-href*="docid="], a[href*="docid="], a[href*="doc="]');
                const recordId = this.getPnxRecordId();
                console.log('### recordId=', recordId);

                console.log('### pnx=', pnx);

                const pageTitle = document.querySelector('nde-record-title h3');
                console.log('### pageTitle=', pageTitle?.innerHTML);
                console.log('### pnx.display.title=', pnx?.display?.title);
    ...
    // TODO combine with recordaproblem into common callable, if used
    private getRecordId = () => {
        const item = document.querySelector('.search-result-item');
        const pnx = getPnx(this.searchState(), item);
        if (!pnx) {
        ...
        const trialRecordId = this.getPnxRecordId();
        if (trialRecordId !== null) {
            return trialRecordId;
        }
        // if (!!pnx?.control?.recordid && pnx.control.recordid.length > 0 && pnx.control.recordid[0]) {
        //     console.log('recordid from 1: pnx.control.recordid');
        //     return encodeURIComponent(pnx.control.recordid[0]);
        // }
        //
        // if (!!pnx?.search?.recordid) {
        //     console.log('recordid from 2: pnx?.search?.recordid');
        //     return encodeURIComponent(pnx.search.recordid);
        // }

        if (this.getDocId() !== '') {
            console.log('recordid from 3: docid');
    ...
    private async getTalisDataFromAllApiCalls(listUrls: string[]): Promise<boolean> {
        ...
        console.log('### getTalisDataFromAllApiCalls local cache pnxId=', pnxId);

        const listUrlsToCall = listUrls.filter(url => url.startsWith('http'));
        console.log('### getTalisDataFromAllApiCalls; listUrlsToCall=', listUrlsToCall);
        ...
        console.log('### add newcourseList to cache', newcourseList);
        return coursesFound;
    }
    ...
    private removePreviousSidebar = () => {
        ...
        previousReadingListSidebar?.length > 0 && console.log('### removePreviousSidebar previousReadingListSidebar=', previousReadingListSidebar);
        ...
        // console.log('### removePreviousSidebar ndeSidebar=', ndeSidebar);
        ...
        if (!!ndeSidebar && !ndeSidebar?.innerHTML?.toString().includes('nde-collapsible-box')) {
            ...
            console.log('### removePreviousSidebar ndeSidebar has no children, remove');
            ndeSidebar.remove();
        }
    }
